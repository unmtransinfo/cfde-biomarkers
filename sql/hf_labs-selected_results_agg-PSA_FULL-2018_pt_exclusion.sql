...
UPDATE jjyang.hf_f_lab_psa_2018_patients jp
        ...
        WHERE jp.patient_id IN (
               SELECT
                        jpd.patient_id
                FROM
                        jjyang.hf_f_lab_psa_2018_pt_dx jpd
                WHERE
                        jpd.diagnosis_description ILIKE '%cancer%'
                        OR jpd.diagnosis_description ILIKE '%tumor%'
                        OR jpd.diagnosis_description ILIKE '%neoplasm%'
                        OR jpd.diagnosis_description ILIKE '%carcinoma%'
        )
        ;
SELECT
        COUNT(jp.patient_id),
