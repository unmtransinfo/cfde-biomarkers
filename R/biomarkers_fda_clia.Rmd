...
knitr::kable(clia_detail_ner_gp)
```
...
mols <- parse.smiles(clia_detail_ner_chem[sample(.N, 20)][["SMILES"]])
...
plot(NA, xlim=c(0, nrow(img)), ylim=c(0, ncol(img)), xlab="", ylab="", xaxt="n", yaxt="n")
...
n_row <- 5
n_col <- 7
moldata <- clia_detail_ner_chem[sample(nrow(clia_detail_ner_chem), n_row*n_col)] #random sample
plot(c(0,1e3), c(0,1e3), pch=1, col="white", axes=FALSE, xlab="", ylab="")
title(main=sprintf("CLIA sample compounds (%d/%d)", nrow(moldata), nrow(clia_detail_ner_chem[!is.na(SMILES)])), line = -3, sub="Depicted with RCDK", cex.main=2)
