...
```{r}
LOINC_RELEASE <- trimws(read_file("loinc_data/loinc_release.txt"))
message(sprintf("LOINC_RELEASE: %s", LOINC_RELEASE))
```
...
## NER on LOINC field: "relatednames".

```{r}
protein_NER_relatednames_file <-  "loinc_data/loinc_chem_names_10_NM_CFDictGeneAndProtein_leadmine.tsv"
...
sprintf("LOINC codes mapped via relatednames text to NER proteins: %d", protein_NER_relatednames[, uniqueN(DocName)])
```
