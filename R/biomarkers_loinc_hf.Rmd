---
title: Human protein biomarkers, with clinical relevance inferred from Cerner HealthFacts
  via LOINC terms
author: "Jeremy Yang"
output:
  html_document:
...
# Cerner HealthFacts 2019 counts of encounter\_ids and patient\_ids for all lab procedures, with LOINC codes.

```{r}
hf_loinc_counts_file <- "cerner_hf_data/hf_lab_loinc_counts_OUT.tsv"
sprintf("Cerner HealthFacts 2019 counts for labs, with LOINC codes: %s", hf_loinc_counts_file)
...
## Group lab procedures into list; aggregate on LOINC codes; discard Cerner lab procedure IDs.
...
write_delim(loinc2geneOrProteinId, "loinc_data/loinc2gp.tsv", delim="\t")
```
...
knitr::kable(protein_hr_loincs_out[1:50], caption=sprintf("LOINCs ranked by occurence: %d LOINCs, %d geneOrProteinIds", protein_hr_loincs_out[, uniqueN(loinc_code)], protein_hr_loincs_out[, uniqueN(geneOrProteinId)]))
```
