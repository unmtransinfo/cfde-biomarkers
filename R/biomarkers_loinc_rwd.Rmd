...
```{r}
protein_hr_loincs_out <- unique(protein_hr_loincs[, .(loinc_code=DocName, geneOrProteinId = ResolvedForm, mnemonics, protein_name=EntityText, encounter_id_count, patient_id_count)])
...
setcolorder(protein_hr_loincs_out, c("loinc_code", "geneOrProteinId", "mnemonics", "protein_names", "encounter_id_count", "patient_id_count"))
```

# Top occurring LOINCs

For now, remove data without resolved protein IDs.
...
protein_hr_loincs_out <- unique(protein_hr_loincs_out[order(-encounter_id_count)])
