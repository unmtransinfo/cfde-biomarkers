...
report_dt <- data.table(gpid = gpids, labs = "", valcount = NaN, units = "", pref_units = "", pref_units_valcount = NaN, mean = NaN, var = NaN)
for (i in 1:length(gpids)) {
  ...
  report_dt$var[i] <- var(hf_results_this[["numeric_result"]], na.rm=T)
}
