---
title: 'Oracle HealthFacts: Laboratory molecular biomarker results analysis: Glycosolated Hemoglobin (HGBA1C) biomarkers'
output:
  html_document:
    df_print: paged
...
Before running this workflow, the following steps are required, in order:
 ...
 - hf_labs-selected_results_agg-HgbA1C_FULL-2018.sql
 - hf_labs-selected_results_agg-HgbA1C_FULL-2018_patients.sql
 - hf_labs-selected_results_agg-HgbA1C_FULL-2018_dx.sql
 - hf_labs-selected_results_agg-HgbA1C_FULL-2018_pt_exclusion.sql
 - hf_labs-selected_results_agg-HgbA1C_FULL-2018_pt_agegroup.sql
 - Export db table hf_f_lab_psa_2018 to hf_f_lab_hgba1c_2018_out.tsv.
 - Export db table hf_f_lab_psa_2018_patients to hf_f_lab_hgba1c_2018_patients_out.tsv.

## Read data file.

```{r echo=F}
ifile <- "oracle_hf_data/hf_f_lab_hgba1c_2018_out.tsv"
hf_lab <- read_delim(ifile, "\t", escape_double=F, show_col_types = F, col_types = cols(.default = col_character(), age_in_years = col_integer(), age_in_days = col_integer(), numeric_result = col_number()))
setDT(hf_lab)
...
## Define biomarker entities of interest.

```{r echo=F}
boi <- list("Hgb A1C" = NULL, "Hgb A1C HPLC" = NULL)
# Entity metadata
boi_metadata <- list("Hgb A1C" = list("entity_name" = "Glycosylated hemoglobin",
                                         "entity_type" = "Protein",
                                         "entity_id" = "3B75",
                                         "entity_id_source" = "PDB"),
                     "Hgb A1C HPLC" = list("entity_name" = "Glycosylated hemoglobin",
                                         "entity_type" = "Protein",
                                         "entity_id" = "3B75",
                                         "entity_id_source" = "PDB"))
i <- 0L
for (lpm in names(boi)) {
  i <- i + 1
  message(sprintf("%d. Biomarker Lab Test of interest: \"%s\"", i, lpm))
}
# Name of this biomarker group/topic.
boi_group_name <- "HgbA1C"
```

## Describe datasets for biomarker entities of interest.

```{r echo=F}
report_dt <- data.table(i = 1:length(boi), entity_name="", entity_type="", entity_id="", entity_id_source="", lab = "", valcount = NaN, all_units = "", pref_units = "", pref_units_valcount = NaN, mean = NaN, median = NaN, sd = NaN)
i <- 0L
for (lpm in names(boi)) {
  hf_lab_this <- hf_lab[lab_procedure_mnemonic == lpm]
  i <- i + 1
  report_dt$entity_name[i] <- boi_metadata[[lpm]][["entity_name"]]
  report_dt$entity_type[i] <- boi_metadata[[lpm]][["entity_type"]]
  report_dt$entity_id[i] <- boi_metadata[[lpm]][["entity_id"]]
  report_dt$entity_id_source[i] <- boi_metadata[[lpm]][["entity_id_source"]]
...
report_dt[, .(entity_name, entity_type, entity_id, entity_id_source, lab, pref_units, mean, median, sd, pref_units_valcount)] %>% 
  kbl(caption = sprintf("Summary of data for biomarker entities of interest (%d)", length(boi)), booktabs=T, digits=2, align = "lllllccccc", col.names = c("entity_name", "entity_type", "entity_id", "entity_id_source", "lab", "units", "mean", "median", "sd", "n")) %>%
    kable_styling(c("striped", "hover"), full_width = F, position = "left")
```
...
subplot(plots, nrows=2, shareX=T, shareY=F, titleX=T, titleY=T, margin=0.03) %>%
...
```{r echo=F}
ifile <- "oracle_hf_data/hf_f_lab_hgba1c_2018_patients_out.tsv"
hf_pt <- read_delim(ifile, "\t", escape_double=F, show_col_types=F, col_types = cols(.default = col_character(), age_mid_year = col_number(), exclusion_flag = col_logical()))
setDT(hf_pt)
...
Also remove patients with age < 20 (too few, and normally not relevant to prostate/PSA issues).
...
### Before and after applying exclusion criteria (diagnosis related to prostate cancer or any cancer).
### Before:

```{r echo=F}
message(sprintf("Patients: %d; Not-excluded: %d (%.1f%%)", 
        hf_pt[, .N], 
        hf_pt[exclusion_flag==F, .N], 
        100 * hf_pt[exclusion_flag==F, .N] / hf_pt[, .N]))
hf_pt <- hf_pt[exclusion_flag==F]
message(sprintf("Patients (after exclusion): %d", hf_pt[, .N]))
...
fig <- plot_ly(alpha = 0.8, xbins = list(size=5)) %>% 
	...
	layout(barmode = "group",
				 title = sprintf("Labs: (%s) cohort:<br>Histogram of patient count by age (N_total=%d)", paste(names(boi), collapse=", "), hf_pt[, .N]),
...
report_stratstat <- data.table(i = 1:(2*n_agegroup*length(boi)), entity_name = "", entity_type = "", entity_id = "", entity_id_source = "", lab = "", agegroup = "", sex = "", units = "", mean = NaN, min = NaN, max = NaN, q1 = NaN, q2_median = NaN, q3 = NaN, sd = NaN, n = NaN)
i <- 0L
for (lpm in names(boi)) {
  ...
  for (a in levels(hf_lab_pt_this[, agegroup])) {
    for (g in levels(hf_lab_pt_this[, gender])) {
      ...
      report_stratstat$entity_name[i] <- boi_metadata[[lpm]][["entity_name"]]
      report_stratstat$entity_type[i] <- boi_metadata[[lpm]][["entity_type"]]
      report_stratstat$entity_id[i] <- boi_metadata[[lpm]][["entity_id"]]
      report_stratstat$entity_id_source[i] <- boi_metadata[[lpm]][["entity_id_source"]]
      report_stratstat$lab[i] <- lpm
      hf_lab_pt_this_this <- hf_lab_pt_this[agegroup == a & gender == g]
      message(sprintf("%d. agegroup: %s; gender: %s; N = %d", i, a, g, hf_lab_pt_this_this[, .N]))
      hf_lab_pt_this_this[, unit_display := as.factor(unit_display)]
      units <- hf_lab_pt_this_this[, unit_display]
      if (length(levels(units))>1) {
        tbl <- data.table(table(hf_lab_pt_this_this[, unit_display]))[order(-N)]
        units_preferred <- tbl[["V1"]][1]
        message(sprintf("WARNING: (a=%s, g=%s) multiple units: (%s); units_preferred: %s (%d / %d)", a, g, paste(levels(units), collapse = ", "), units_preferred,  tbl[["N"]][1], sum(tbl[["N"]])))
      } else {
        units_preferred <- levels(units)[1]
      }
      report_stratstat$units[i] <- units_preferred
      values <- hf_lab_pt_this_this[unit_display == units_preferred, numeric_result]
...
report_stratstat[, .(entity_name, entity_type, entity_id, entity_id_source, lab, agegroup, sex, units, min, max, mean, sd, q1, q2_median, q3, n, boxplot)] %>%
  ...
  column_spec(17, image = spec_boxplot(vals)) %>%
  row_spec(0, color = 'navy', background = 'orange')
ofile <- sprintf("oracle_hf_data/stratified_stats_report_%s.tsv", boi_group_name)
write_delim(report_stratstat[, .(entity_name, entity_type, entity_id, entity_id_source, lab, agegroup, sex, units, min, max, mean, sd, q1, q2_median, q3, n)], ofile, "\t")
