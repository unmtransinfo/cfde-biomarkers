...
tbl <- tbl[order(-N)]
data.table(tbl) %>% 
  kbl(caption = "Lab tests in dataset, with value counts") %>%
...
report_dt[, .(lab, pref_units, pref_units_valcount, mean, median, sd)] %>% 
  kbl(caption = sprintf("Summary of data for biomarker[s] of interest (%d)", length(boi)), digits=2, align = "lccccc", col.names = c("lab", "units", "N", "mean", "median", "sd")) %>%
    kable_styling(c("striped", "hover"), full_width = F, position = "left")
```
...
for (lpm in names(boi)) {
  ...
  kbl(caption = sprintf("%s: distribution percentiles", lpm), align = "cc") %>%
...
for (lpm in names(boi)) {
  ...
  qtl <- quantile(values, probs = seq(0, 1, .1))
  data.table(percentiles = names(qtl), values = qtl) %>% 
    kbl(caption = sprintf("%s: distribution percentiles", lpm)) %>%
    kable_styling(full_width = F)
}
```
...
tbl <- tbl[order(-n_pt)]
tbl %>%
  kbl(caption = sprintf("Labs: (%s) cohort, pre-exclusion, Gender distribution", paste(collapse=", ", names(boi)))) %>%
  kable_styling(full_width = F)
```
...
tbl <- tbl[order(-n_pt)]
tbl %>%
  kbl(caption = sprintf("Labs: (%s) cohort, Gender distribution", paste(names(boi), collapse=", "))) %>%
  kable_styling(full_width = F)
```
...
tbl <- tbl[order(-n_pt)]
tbl %>%
  kbl(caption = sprintf("Labs: (%s) cohort, Patient_type distribution", paste(names(boi), collapse=", "))) %>%
  kable_styling(full_width = F)
```
...
tbl <- tbl[order(-n_pt)]
tbl %>%
  kbl(caption = sprintf("Labs: (%s) cohort, Race distribution", paste(names(boi), collapse=", "))) %>%
  kable_styling(full_width = F)
```
...
qtl <- quantile(values, probs = seq(0, 1, .1))
data.table(percentiles = names(qtl), values = qtl) %>%
  kbl(caption = sprintf("Labs: (%s) cohort, Age_mid_year: distribution percentiles", paste(names(boi), collapse=", ")), digits=1) %>%
  kable_styling(full_width = F)
```
...
write_delim(report_stratstat, "oracle_hf_data/stratified_stats_Troponin_I_report.tsv", "\t")
report_stratstat %>%
  kbl(caption = sprintf("%s: Normal-physiological healthy patient cohort, stratified statistics", lab_procedure_mnemonic_this)) %>%
  kable_styling(full_width = F)
```
