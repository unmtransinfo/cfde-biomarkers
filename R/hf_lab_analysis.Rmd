...
data.table(tbl) %>% kbl() %>%
  ...
  footnote(general_title = "Title:", general = "Lab tests in dataset, with value counts")
```

## Define biomarker[s] of interest.

```{r echo=F}
boi <- c("Troponin I", "Troponin T S/Pl")
for (lab_procedure_mnemonic_this in boi) {
...
```{r echo=F}
report_dt <- data.table(i = 1:length(boi), labs = "", valcount = NaN, units = "", pref_units = "", pref_units_valcount = NaN, mean = NaN, median = NaN, sd = NaN)
i <- 0L
for (lab_procedure_mnemonic_this in boi) {
...
report_dt %>% kbl() %>%
    kable_styling(c("striped", "hover"), full_width = F, position = "left") %>%
    footnote(general_title = "Title: ", general = sprintf("Summary of data for biomarker[s] of interest (%d)", length(boi)))
```
```{r echo=F}
for (lab_procedure_mnemonic_this in boi) {
  hf_lab_this <- hf_lab[lab_procedure_mnemonic == lab_procedure_mnemonic_this]
  values <- hf_lab_this[["numeric_result"]]
  message(sprintf("%s; N = %d; min = %.4f; max = %.4f; mean = %.4f; median = %.4f; sd = %.4f", lab_procedure_mnemonic_this, length(values), min(values), max(values), mean(values), median(values), sd(values)))
