...
report_stratstat <- data.table(i = 1:(2*n_agegroup*length(boi)), lab = "", agegroup = "", sex = "", units = "", mean = NaN, median = NaN, sd = NaN, n = NaN)
i <- 0L
for (lpm in names(boi)) {
...
#write_delim(report_stratstat, sprintf("oracle_hf_data/stratified_stats_report.tsv"), "\t")
report_stratstat[, .(lab, agegroup, sex, units, mean, median, sd, n)] %>%
  ...
  kable_styling(c("striped", "hover"), full_width = T, position="left") %>%
